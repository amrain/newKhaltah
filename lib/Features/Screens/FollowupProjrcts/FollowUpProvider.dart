  ...
  getFollowUp(int idContract) async {
    try{
      ...
      FollowUpModel followUpModel =
          await FollowUpHelper.followUpHelper.getFollowUp(idContract);
      followUp = followUpModel.data;
      loading = false;
      notifyListeners();
      for (int i = 0; i < followUp!.length; i++) {
        controllersVideo[followUp![i].name.toString()] =
            VideoPlayerController.network(
                "${API.imageUrl}${followUp![i].workVideo}");
        controllersVideo[followUp![i].name.toString()]?.addListener(() {
          notifyListeners();
        });
        controllersVideo[followUp![i].name.toString()]?.setLooping(true);
        controllersVideo[followUp![i].name.toString()]?.initialize().then((_) {
          notifyListeners();
        });
